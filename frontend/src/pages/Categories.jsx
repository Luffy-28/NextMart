...
import { fetchAllActiveSubCategory, fetchSubCategoryCount } from '../features/subCategory/subCategoryAction.js';
...
const Categories = () => {
  ...
  const { subCategories, totalSubCategories, loading: subCategoryLoading } = useSelector((state) => state.subCategoryStore);
  const { categories, loading: categoryLoading } = useSelector((state) => state.categoryStore);
  const { products, pagination } = useSelector((state) => state.productStore);

  // Fetch categories, total product count and total subcategory count on mount
  useEffect(() => {
    dispatch(fetchAllActiveCategory());
    dispatch(fetchAllProducts(1,12));
    dispatch(fetchSubCategoryCount());    // sets totalSubCategories in subCategoryStore
  }, [dispatch]);
  ...
  return (
    <div style={{ background: 'var(--nex-bg)', minHeight: '100vh' }}>
      ...
      <div className="container-fluid px-4 px-lg-5 py-5">
        ...
        <div className="row g-4 row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
          {!activeCategory ? (
            categoryLoading ? (
        ...
        {!activeCategory && (
          <div className="nex-glass-card p-4 mt-5">
            <div className="row g-3 text-center">
              {[
                { value: `${pagination.totalItems}`, label: 'Products', icon: 'bi-box-seam' },
                { value: totalSubCategories ? `${totalSubCategories}` : '—', label: 'Subcategories', icon: 'bi-diagram-3' },
                { value: categories?.length ? `${categories.length}` : '—', label: 'Categories', icon: 'bi-grid-3x3-gap' },
